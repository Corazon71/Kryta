...
// --- COMPONENT: Settings View (Updated) ---
    ...
    const handleSave = async () => {
      ...
      try {
        ...
        if (res.status === 'success') {
          ...
          setIsConfigured(true); // <--- TURN SYSTEM ONLINE
          setKey(""); 
        } else {
...
function App() {
  ...
  const handlePlan = async (e) => {
    ...
    // --- STRICT GATEKEEPING ---
    if (!isConfigured) {
        showToast("ACCESS DENIED: Neural Link Required", "error");
        return;
    }
    ...
    <div className="h-screen w-screen bg-background text-gray-200 font-sans overflow-hidden relative selection:bg-primary/30">
      ...
      <div className="absolute bottom-0 left-0 w-full p-8 flex justify-between items-end z-20 pointer-events-none">
        ...
        <div className="flex-1 max-w-2xl mx-4 pointer-events-auto">
             <div className={`bg-surface/80 backdrop-blur-md border rounded-2xl p-1 flex items-center shadow-2xl ring-1 transition-all ${
                 isConfigured ? "border-border ring-white/5" : "border-red-500/30 ring-red-500/20"
             }`}>
                <div className={`px-4 animate-pulse ${isConfigured ? "text-primary" : "text-red-500"}`}>
                    {isConfigured ? <Terminal size={18} /> : <AlertTriangle size={18} />}
                </div>
                <input 
                    type="text"
                    className="flex-1 bg-transparent border-none outline-none text-white placeholder-gray-600 h-12 font-medium"
                    placeholder={loading ? "DAEMON is thinking..." : isConfigured ? "Enter directive..." : "SYSTEM OFFLINE // CONFIGURATION REQUIRED"}
                    value={goal}
             ...
             <div className="text-center mt-2">
                <span className={`text-[10px] uppercase tracking-widest ${isConfigured ? "text-gray-600" : "text-red-500"}`}>
                    {isConfigured ? "Daemon v1.0 • Ready for Input" : "Connection to Neural Net Severed"}
                </span>
             </div>
        </div>
        <div className="flex gap-2 pointer-events-auto bg-surface/50 backdrop-blur-md p-2 rounded-2xl border border-border">
            ...
            <button 
                ...
                className={`p-3 rounded-xl transition-all relative ${view === 'settings' ? 'bg-primary text-white shadow-lg' : 'hover:bg-white/5 text-gray-400'}`}
            >
                <Settings size={20} />
                {!isConfigured && (
                    <span className="absolute top-2 right-2 w-2 h-2 bg-red-500 rounded-full animate-pulse shadow-[0_0_8px_red]"></span>
                )}
            </button>
        </div>
      </div>
      ...
      <div className="absolute inset-0 pt-32 pb-32 px-8 overflow-y-auto scrollbar-hide z-10 flex flex-col items-center">
        {view === 'analytics' ? (
          ...
          <SettingsView showToast={showToast} setIsConfigured={setIsConfigured} />
        ) : (
          <>
            {tasks.length === 0 && !loading && (
              <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="text-center mt-20">
                <h1 className="text-5xl font-bold text-white mb-4 tracking-tight">
                    {isConfigured ? "Good Morning, Operator." : <span className="text-gray-500">SYSTEM OFFLINE.</span>}
                </h1>
                <p className="text-gray-500 text-lg max-w-md mx-auto">
                    {isConfigured 
                        ? "The system is online. Input your primary objective below to initiate the protocol."
                        : "Configuration required. Please access Settings to establish Neural Link."
                    }
                </p>
              </motion.div>
            )}
